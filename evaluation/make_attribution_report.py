...
function lineChart(id,rs,series,log=false,yBounds=null){const svg=$(id),W=1100,H=390,L=64,R=20,T=18,B=46,n=rs.length;if(n<2){svg.innerHTML="";return}let vals=series.flatMap(s=>s.v.filter(Number.isFinite));let lo=yBounds?yBounds[0]:Math.min(...vals),hi=yBounds?yBounds[1]:Math.max(...vals);if(log){lo=Math.log(Math.max(lo*.98,1e-8));hi=Math.log(hi*1.02)}if(hi===lo){hi+=1;lo-=1}const pW=W-L-R,pH=H-T-B,x=i=>L+pW*i/(n-1),y=v=>T+pH*(1-((log?Math.log(v):v)-lo)/(hi-lo)),vAt=f=>log?Math.exp(lo+(hi-lo)*f):lo+(hi-lo)*f,span=Math.abs(vAt(1)-vAt(0)),dec=span>=50?0:span>=5?1:span>=.5?2:3,fa=v=>Number.isFinite(v)?v.toFixed(dec):"";let out="";for(let j=0;j<5;j++){const yy=T+pH*j/4,v=vAt(1-j/4);out+=`<line x1="${L}" x2="${W-R}" y1="${yy}" y2="${yy}" stroke="#e7ecf2"/>`;out+=`<text x="${L-8}" y="${yy+4}" text-anchor="end" font-size="11" fill="#64748b">${fa(v)}</text>`}out+=`<line x1="${L}" y1="${T}" x2="${L}" y2="${T+pH}" stroke="#94a3b8"/><line x1="${L}" y1="${T+pH}" x2="${W-R}" y2="${T+pH}" stroke="#94a3b8"/>`;for(const s of series)out+=`<polyline fill="none" stroke="${s.c}" stroke-width="1.7" points="${s.v.map((v,i)=>Number.isFinite(v)?`${x(i)},${y(v)}`:"").join(" ")}"/>`;out+=series.map((s,i)=>`<text x="${L+10+i*180}" y="${T+14}" fill="${s.c}" font-size="12">${s.n}</text>`).join("");for(let j=0;j<5;j++){const idx=Math.min(n-1,Math.round((n-1)*j/4)),xx=x(idx);out+=`<line x1="${xx}" y1="${T+pH}" x2="${xx}" y2="${T+pH+4}" stroke="#94a3b8"/><text x="${xx}" y="${H-14}" text-anchor="middle" font-size="11" fill="#64748b">${rs[idx].date}</text>`}svg.innerHTML=out}
function waterfall(rs){const c=linked(rs),arr=COMPONENTS.map(k=>({k,v:c[k]})),svg=$("waterfall"),W=1000,H=360,L=150,R=20,T=20,B=40,max=Math.max(...arr.map(x=>Math.abs(x.v)),.001),x0=L+(W-L-R)/2,scale=(W-L-R)/2/max,pH=H-T-B;let out=`<line x1="${x0}" x2="${x0}" y1="${T}" y2="${H-B}" stroke="#94a3b8"/>`;arr.forEach((a,i)=>{const y=T+i*pH/arr.length+5,h=pH/arr.length-10,x=a.v>=0?x0:x0+a.v*scale,w=Math.abs(a.v)*scale;out+=`<text x="${L-8}" y="${y+h*.72}" text-anchor="end" font-size="12">${LABELS[a.k]}</text><rect x="${x}" y="${y}" width="${w}" height="${h}" fill="${COLORS[a.k]}"><title>${LABELS[a.k]} ${pct(a.v)}</title></rect><text x="${a.v>=0?x+w+5:x-5}" y="${y+h*.72}" text-anchor="${a.v>=0?'start':'end'}" font-size="11">${pct(a.v)}</text>`});out+=`<line x1="${L}" y1="${H-B}" x2="${W-R}" y2="${H-B}" stroke="#94a3b8"/>`;for(let j=0;j<5;j++){const v=(j/4*2-1)*max,xp=x0+v*scale;out+=`<line x1="${xp}" y1="${H-B}" x2="${xp}" y2="${H-B+4}" stroke="#94a3b8"/><text x="${xp}" y="${H-B+16}" text-anchor="middle" font-size="11" fill="#64748b">${pct(v)}</text>`}svg.innerHTML=out;return c}
