...
def add_key_variables(spy_intra_data: pd.DataFrame, dividends: pd.DataFrame) -> tuple[pd.DataFrame, np.ndarray]:
    """Build the strategy feature set from intraday and dividend data."""
...
def run_backtest(
    ...
    """Run the intraday momentum backtest on the prepared feature data."""
    daily_groups = df.groupby("day")
    ...
    for d in range(1, len(all_days)):
        ...
        # Intraday signal: long above upper band and VWAP, short below lower band and VWAP.
        ...
        if sizing_type == "vol_target":
            ...
            raise ValueError("sizing_type must be 'vol_target' or 'full_notional'.")

        # Only rebalance on selected minute buckets (e.g., every 30 minutes).
        ...
        exposure = (
            pd.Series(filled_values, index=current_day_data.index).shift(1).fillna(0).values
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Backtesting BeatTheMarket 2008-2026")
    ...
    parser.add_argument(
        "--output-aum-png",
        type=str,
        default="spy_momentum_equity_curve_2008_2026_paper.png",
        help="Output AUM chart path",
    )
    parser.add_argument(
        "--output-return-png",
        type=str,
        default="spy_momentum_cumreturn_2008_2026_paper.png",
