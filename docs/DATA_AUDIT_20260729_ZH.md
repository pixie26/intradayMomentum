...
## 十三、分红 —— 74 个事件干净，raw 价格获证据支持

74 个季度分红（2008-03-20 → 2026-06-18，0.48–1.993 美元），0 冲突、
0 重复、全部落在交易日。**价格未复权证据**：除息日平均隔夜 gap
−0.276% vs 非除息日基准 +0.027%；把分红加回后 gap 变为 +0.181%，更贴近
基准 → `evidence_supports_raw_prices=true`（证据而非证明）。引擎只在特征
层做 `prev_close − dividend` 调整，原始 OHLC 不动。
清理后文件：`spy_dividends_clean.csv`（sha256
...
| 分红 | 74 事件干净；价格 raw | 特征层调整 prev_close |
